...
sfo_feb_flights %>% group_by(carrier) %>% summarise(iqr_dd = IQR(arr_delay), median_dd = median(arr_delay), n = n())
```
...
nycflights <- nycflights %>% mutate(arr_type = ifelse(arr_delay < 5, "on time", "delayed"))
nycflights <- nycflights %>% mutate(dep_type = ifelse(dep_delay < 5, "on time", "delayed"))
nycflights %>% summarise(ot_dep_arr_rate = sum(arr_type == "delayed") / sum(dep_type == "on time")) 
```